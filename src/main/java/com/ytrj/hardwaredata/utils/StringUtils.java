...
public class StringUtils extends org.apache.commons.lang3.StringUtils{
    ...
    public static String toDoubleDigit(int number) {
        if (number >= 0 && number < 10) {
            return String.valueOf("0") + String.valueOf(number);
        }
...
//    }
    ...
    public static String ifEmpty(String str, String defaultStr){ //如果字符串为空，则返回默认字符串
        return StringUtils.isEmpty(str) ? defaultStr : str;
    }

    /**
     * 从header中获取值，当第一个为空的时候，尝试获取第二个
     * @return
     */
    public static String getAnotherForHeaderParam(HttpServletRequest request, String poss1, String poss2){
        String result = request.getHeader(poss1);
        if(StringUtils.isEmpty(result)){
            result = request.getHeader(poss2);
        }
        return result;
    }
    ...
    public static String concatStr(String beginStr, String endStr){
        if(StringUtils.isBlank(endStr)) return null;
